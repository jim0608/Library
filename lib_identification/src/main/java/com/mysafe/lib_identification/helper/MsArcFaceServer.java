...
import com.mysafe.lib_identification.Constants;
...
public class MsArcFaceServer {
    ...
    //region 单例
    private static volatile MsArcFaceServer faceServer = null;

    public static MsArcFaceServer getInstance() {
        if (faceServer == null) {
            synchronized (MsArcFaceServer.class) {
                if (faceServer == null) {
                    faceServer = new MsArcFaceServer();
                }
            }
        }
        return faceServer;
    ...
    public synchronized void activeOnlineEngine(Context context) {
        int activeResult = FaceEngine.activeOnline(context, Constants.ARC_ACTIVE_KEY_PUBLIC, Constants.ARC_APP_ID, Constants.ARC_SDK_KEY);
        if (activeResult == ErrorInfo.MOK) {
            Log.i(TAG, "activeOnline success");
            initFaceEngine(context);
        } else if (activeResult == ErrorInfo.MERR_ASF_ALREADY_ACTIVATED) {
    ...
    public void initFaceEngine(Context context) {
        if (faceEngine == null && context != null) {
            ...
            } else if (engineCode == ErrorInfo.MERR_ASF_NOT_ACTIVATED) {
                //引擎激活失败，重新去激活
                faceEngine = null;
            } else {
                faceEngine = null;
     ...
     * @param context                上下文对象
     */
    private void initFaceList(final Context context) {
        new Thread(() -> {
            synchronized (MsArcFaceServer.this) {
                if (faceRegisterInfoList != null) {
                    return;
                }
                faceRegisterInfoList = FaceDatabase.getDatabase(context).faceDao().getAllFaces();
            }
        }).start();
    }
